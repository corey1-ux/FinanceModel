...
function autoCalculateAssumptions() {
  ...
  const { profile, historicalData } = lastFetchedData;
...
function renderCharts(historicalData) {
    ...
    if (!historicalData || !historicalData.incomeStatements?.length) { chartsContainer.style.display = 'none'; return; }
    if (revenueChart) revenueChart.destroy(); if (fcfChart) fcfChart.destroy();
    const incomes = [...historicalData.incomeStatements].reverse(); const cashflows = [...historicalData.cashflowStatements].reverse();
    const labels = incomes.map(item => item.calendarYear);
    const revenueData = incomes.map(item => item.revenue / 1e9); const fcfData = cashflows.map(item => ((item.operatingCashFlow - item.capitalExpenditure) / 1e9));
    const revenueCtx = document.getElementById('revenue-chart').getContext('2d');
    revenueChart = new Chart(revenueCtx, { type: 'bar', data: { labels: labels, datasets: [{ label: 'Annual Revenue ($B)', data: revenueData, backgroundColor: 'rgba(54, 162, 235, 0.6)', borderColor: 'rgba(54, 162, 235, 1)', borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: true } });
    const fcfCtx = document.getElementById('fcf-chart').getContext('2d');
    fcfChart = new Chart(fcfCtx, { type: 'bar', data: { labels: labels, datasets: [{ label: 'Free Cash Flow ($B)', data: fcfData, backgroundColor: 'rgba(75, 192, 192, 0.6)', borderColor: 'rgba(75, 192, 192, 1)', borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: true } });
    chartsContainer.style.display = 'block';
}
function performDCFCalculation(data) {
    const fcfMargin = data.fcfMargin; const currentFCF = data.currentRevenue * fcfMargin;
    const projections = { revenues: [], fcf: [], pv: [] }; let totalPVFCF = 0; let lastProjectedRevenue = data.currentRevenue;
    for (let year = 1; year <= 10; year++) {
        const growthRate = year <= 5 ? data.revenueGrowth15 : data.revenueGrowth610; const revenue = lastProjectedRevenue * (1 + growthRate); lastProjectedRevenue = revenue;
        const fcf = revenue * fcfMargin; const pvFactor = 1 / Math.pow(1 + data.discountRate, year); const pv = fcf * pvFactor;
        if (year <= 10) { projections.revenues.push(revenue); projections.fcf.push(fcf); projections.pv.push(pv); }
        totalPVFCF += pv;
    }
    const lastYearFCF = lastProjectedRevenue * fcfMargin; const terminalFCF = lastYearFCF * (1 + data.terminalGrowth);
    const denom = data.discountRate - data.terminalGrowth; const terminalValue = (denom <= 0.001) ? terminalFCF * 15 : terminalFCF / denom;
    const pvTerminal = terminalValue / Math.pow(1 + data.discountRate, 10); const enterpriseValue = totalPVFCF + pvTerminal;
    const netDebt = data.totalDebt - data.cashEquivalents; const equityValue = enterpriseValue - netDebt;
    const fairValuePerShare = data.sharesOutstanding > 0 ? equityValue / data.sharesOutstanding : 0;
    projections.terminalFCF = terminalFCF; projections.pvTerminal = pvTerminal;
...
function updateResults(results, normalizedFcf) {
    ...
    document.getElementById('enterprise-value').textContent = `$${Math.round(results.enterpriseValue).toLocaleString()}M`; document.getElementById('net-debt').textContent = `$${Math.round(results.netDebt).toLocaleString()}M`;
    document.getElementById('equity-value').textContent = `$${Math.round(results.equityValue).toLocaleString()}M`; document.getElementById('fair-value').textContent = `$${results.fairValuePerShare.toFixed(2)}`;
    if (isFiniteNumber(currentData.currentPrice) && currentData.currentPrice > 0) { const upside = ((results.fairValuePerShare - currentData.currentPrice) / currentData.currentPrice * 100); const comparison = upside >= 0 ? `${upside.toFixed(1)}% upside` : `${Math.abs(upside).toFixed(1)}% downside`; const el = document.getElementById('vs-current-price'); el.textContent = `vs Current $${currentData.currentPrice.toFixed(2)}: ${comparison}`; el.className = upside >= 0 ? 'positive' : 'negative'; }
}
...
function loadSampleData() { document.getElementById('ticker-input').value = 'AAPL'; document.getElementById('revenue-growth-1-5').value = '8'; document.getElementById('revenue-growth-6-10').value = '5'; document.getElementById('terminal-growth').value = '2.5'; document.getElementById('discount-rate').value = '9.0'; document.getElementById('fcf-margin').value = '25.0';}
...
function getIndustryGrowthEstimates(sector, industry) { const sectorGrowth = { 'Technology': { nearTerm: 12, longTerm: 6 }, 'Healthcare': { nearTerm: 8, longTerm: 5 }, 'Financial Services': { nearTerm: 6, longTerm: 4 }, 'Consumer Cyclical': { nearTerm: 7, longTerm: 4 }, 'Consumer Defensive': { nearTerm: 4, longTerm: 3 }, 'Industrials': { nearTerm: 6, longTerm: 4 },'Energy': { nearTerm: 5, longTerm: 3 }, 'Utilities': { nearTerm: 3, longTerm: 2 }, 'Real Estate': { nearTerm: 4, longTerm: 3 }, 'Materials': { nearTerm: 5, longTerm: 3 }, 'Communication Services': { nearTerm: 8, longTerm: 5 } }; const industryAdjustments = { 'Software': 1.3, 'Semiconductors': 1.2, 'Biotechnology': 1.4, 'Airlines': 0.8, 'Banks': 0.9 }; const baseGrowth = sectorGrowth[sector] || { nearTerm: 6, longTerm: 4 }; const industryMultiplier = industryAdjustments[industry] || 1.0; return { nearTerm: baseGrowth.nearTerm * industryMultiplier, longTerm: baseGrowth.longTerm * industryMultiplier }; }
